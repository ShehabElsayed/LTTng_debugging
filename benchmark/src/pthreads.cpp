...
//Shehab-- Add zsim pseudo instructions
//#include "/aenao-99/elsayed9/benchmarks/parsec_3.0/zsim_pseudo_macros.h"
#include "/aenao-99/elsayed9/benchmarks/parsec_3.0/pkgs/instrumentation/inst_macros.h"
#ifdef ZSIM_SYNC_INST
//#include "/aenao-99/elsayed9/benchmarks/parsec_3.0/zsim_pseudo_macros.h"
#include "/localhome/elsayed9/zsim/misc/hooks/zsim_hooks.h"
#include <pthread.h>
#endif
#ifdef LTTNG_INST
#include "/aenao-99/elsayed9/benchmarks/parsec_3.0/pkgs/instrumentation/lttng_tp/tp.h"
#endif
//Shehab--
    ...
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
      {
        ...
        for(int j = 0; j < np2; ++j)
        {
          ...
          if(border[index]) {
						BEF_LCK(lock_id[index][CELL_MUTEX_ID], 1)
            pthread_mutex_lock(&mutex[index][CELL_MUTEX_ID]);
						AFT_LCK(lock_id[index][CELL_MUTEX_ID], 1)
					}
          ...
          if(border[index]) {
            pthread_mutex_unlock(&mutex[index][CELL_MUTEX_ID]);
						END_LCK(lock_id[index][CELL_MUTEX_ID], 1)
					}
            ...
            for(int iparNeigh = 0; iparNeigh < numNeighPars; ++iparNeigh)
            {
              ...
              if(&neigh->p[iparNeigh % PARTICLES_PER_CELL] < &cell->p[ipar % PARTICLES_PER_CELL])
              {
                ...
                if(distSq < hSq)
                {
                  ...
                  if( border[index])
                  {
										BEF_LCK(lock_id[index][ipar % MUTEXES_PER_CELL], 4)
                    pthread_mutex_lock(&mutex[index][ipar % MUTEXES_PER_CELL]);
										AFT_LCK(lock_id[index][ipar % MUTEXES_PER_CELL], 4)
                    cell->a[ipar % PARTICLES_PER_CELL] += acc;
                    pthread_mutex_unlock(&mutex[index][ipar % MUTEXES_PER_CELL]);
										END_LCK(lock_id[index][ipar % MUTEXES_PER_CELL], 4)
                  }
                  else
                    cell->a[ipar % PARTICLES_PER_CELL] += acc;
                  ...
                  if( border[indexNeigh])
                  {
										BEF_LCK(lock_id[indexNeigh][iparNeigh % MUTEXES_PER_CELL], 5)
                    pthread_mutex_lock(&mutex[indexNeigh][iparNeigh % MUTEXES_PER_CELL]);
										AFT_LCK(lock_id[indexNeigh][iparNeigh % MUTEXES_PER_CELL], 5)
                    neigh->a[iparNeigh % PARTICLES_PER_CELL] -= acc;
                    pthread_mutex_unlock(&mutex[indexNeigh][iparNeigh % MUTEXES_PER_CELL]);
										END_LCK(lock_id[indexNeigh][iparNeigh % MUTEXES_PER_CELL], 5)
                  }
                  else
                    neigh->a[iparNeigh % PARTICLES_PER_CELL] -= acc;
                }
...
{
	//Shehab-- LTTng thread beginning and end instrumentation is now added to
	//			-- the main LTTng-ust and not the local tracepoints
	#ifndef LTTNG_INSTT
	TRD_BEG((int)pthread_self())
	#endif
	//Shehab--
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
        ...
        //Shehab-- Add space to fix warning with newer gcc7.2
				std::cout << "PARSEC Benchmark Suite Version " __PARSEC_XSTRING(PARSEC_VERSION) << std::endl << std::flush;
				//Shehab--
#else
